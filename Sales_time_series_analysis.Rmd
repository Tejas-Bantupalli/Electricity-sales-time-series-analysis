...
Calculate Mean Absolute Error (MAE) and Mean Absolute Percentage Error (MAPE) to evaluate the model's performance.

## Calculate MAE and MAPE for evaluation

```{r message = FALSE, error= FALSE}
observed <- ne_sales_data$y[(nrow(ne_sales_data) - 11):nrow(ne_sales_data)]
predicted <- forecast$yhat[(nrow(forecast) - 23):(nrow(forecast) - 12)]

mae <- mean(abs(observed - predicted))
mape <- mean(abs((observed - predicted) / observed)) * 100

list(MAE = mae, MAPE = mape)
```
